#ifndef __GETMESHIOS_H__
#define __GETMESHIOS_H__
// File i/o replaced with fstreams
#include "genrand.h"
#include "mainhead.h"
#include <math.h>
...
bool get_mesh_dim(int &Ntets, int &Nnodes){
	...
	std::ifstream meshin; //FILE* meshin;
	meshin.open(MESHFILE, std::ios::in); //meshin = fopen(MESHFILE, "r");

	if (!meshin.is_open()) {
		printf("\nInput mesh file not found!"); 
		return false;
	}

	printf("Reading input mesh file for dimensions ... \t");

	//read total number of nodes and tetrahedras
	meshin >> Nnodes >> Ttot; //fscanf(meshin, "%d %d\n", &Nnodes, &Ttot);
...
bool get_mesh(NodeArray &i_Node, TetArray &i_Tet, int Ntets, int Nnodes){
	...
	printf("Reading input mesh file ... \t");
	...
	for (int i = 0; i<(Ttot - Ntets); i++){
		...
		meshin.getline(dummystring, 100);
	}
