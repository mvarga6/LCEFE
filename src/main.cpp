...
{
	SimulationDelegate * delegate = new SimulationDelegate();
	ProcessInputResult resultResult = delegate->ProcessUserInput(argc, argv);
	InitializeResult initResult = delegate->Initialize();
	return delegate->Run();
}
